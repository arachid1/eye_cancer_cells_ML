...
def K_MEANS(vectors,path_all,config,phase,_bagnet_filter=0.99,_bagnet_batch=100000):
...
def test_save_img(epoch,assignment, config, phase):
	...
	for i in assignment:
		...
		elif phase == 'normal' or phase == 'coarse_to_fine':
			target = float(target)
...
def M_new_assignment(_mean,_std):
...
def T_SNE(data,assignment,config, phase):
	...
	X_tsne = tsne.fit_transform(data.reshape(-1,data.shape[-1]))
	for i in range(X_tsne.shape[0]):
		_random = random.random() if phase == "bagnet" else 1
		if _random > 0.99:
...
def coarse_to_fine_cluster_enlarge(config,epoch,M):
	if len(config['coarse_to_fine_epoch']) > 0:
		if epoch == config['coarse_to_fine_epoch'][0]:
			config['n_cluster'] = config['n_cluster'] + config['coarse_to_fine_n_cluster'][0]
			M = torch.cat((M,torch.zeros(M.shape[0],config['coarse_to_fine_n_cluster'][0])),dim=1)
			del config['coarse_to_fine_n_cluster'][0]
			del config['coarse_to_fine_epoch'][0]
	return config, M
